% Paper template - Fission product defect energies by Alex Kenich, Mark Wenman and Robin Grimes
% Imperial College London, Department of Materials
% Most of the required packages are standard and should be provided by typical TeX installations
...
\hypersetup{allcolors=[rgb]{0.1,0.2,0.5}, colorlinks=true}
...
The defect equilibria of fission products Te, I, Xe and Cs and their defect volumes are predicted in tetragonal \zirconia\ using density functional theory calculations. Dopant fission product concentrations of 10$^{-5}$ to 10$^{-3}$ atoms per formula unit of \zirconia\ are examined. These data are presented in Brouwer diagrams to show the calculated defect concentrations between oxygen partial pressures of 10$^{-35}$ and 10$^{0}$ atm. The dominant Te defects in \zirconia\ are \ch{Te^{**}_{O}} and \ch{Te^{'''}_{Zr}}, with defect volumes relative to the non-defective crystal of 2.74 \r{A}$^{3}$ and 59.93 \r{A}$^{3}$, respectively. Upon $\beta$- decay, these defects become \ch{I^{*}_{O}} and \ch{I^{'''}_{Zr}}, with defect volumes of 29.90 \r{A}$^{3}$ and 87.07 \r{A}$^{3}$, respectively. Upon a second $\beta$- decay, \ch{Xe^{**}_{O}}, \ch{Xe^{'''}_{Zr}} and \ch{Xe^{''''}_{Zr}} are produced, with defect volumes of 9.02 \r{A}$^{3}$, 91.65 \r{A}$^{3}$ and 115.50 \r{A}$^{3}$ respectively. The large positive volumes of these defects generate stresses in the \zirconia\ lattice, and if produced in significant quantities (such as during a power ramp), could promote crack formation. PCI failures, as reported in the literature, exhibit time delays between reactor power ramping and failure which are in line with the decay rate of Te and I isotopes. This has implications for fuel and cladding designs which are resistant to SCC and PCI.

%PCI failures in nuclear fuel claddings occur after power ramps, typically with a time delay of tens of minutes. The exact mechanism of failure is not known, however, it is widely believed that fission product SCC is involved in the process to some extent. The internal oxide layer of zirconium-based fuel claddings is protective, and so this layer must be breached before corrosive species can attack the metal underneath. Iodine is one of the main corrosive fission products produced in the fuel, though it is also produced in significant quantities by the decay of Te. Many Te fission product isotopes have half-lives under an hour and high fission yields, making them a large contributor to the iodine inventory during power ramps. Iodine nuclei are also unstable and may decay during the time delay into Xe, with the decay chain continuing to Cs over longer time spans. Fission predominantly occurs at the rim of the fuel pellet near the cladding, leading to fission product implantation within the oxide layer. It is important to determine the effect this can have on the oxide's protective capacity. We therefore examine the defect equilibria and report defect volumes of the fission products Te, I, Xe and Cs in the internal oxide layer of fuel cladding using a quantum mechanical modelling approach.
...
% the time delay before failure in pci related failures is an important thing to talk about. cite that paper which you bookmarked. Talk about how the 
...
Nuclei produced during fission are typically neutron-rich, resulting in decay modes such as $\beta-$ or neutron emission. In the case of tellurium nuclei, most decay into iodine, which then decays into xenon with different half-lives depending on the isotope. An example of such a decay chain is shown in Figure \ref{fig:decaychain} using Te$_{135}$. Several of these isotopes' fission yield and decay data are shown in Table \ref{table:decaydata}. The decay chain continues with xenon nuclei decaying into caesium, many isotopes of which have half lives measured in years. At this point, conventional power reactor fuel is retired long before a significant proportion of the caesium decays into barium. For this reason we only consider the elements tellurium through caesium in this study. \\

A large fraction of thermal fission events occur in the outer rim of a fuel pellet. Given a fission product penetration depth of up to 8 $\mu$m in \zirconia\ \cite{degueldre2001behaviour}, a large proportion of fission products implant within the oxide. It has also been observed in PCI failures that there is a time delay on the order of 10 minutes between the power ramping of the reactor and the subsequent breach of the cladding \cite{bergenlid1980experimental, wood1983effects, pankaskie1981mechanistic}. This time delay is in line with the half-lives of many Te and I isotopes (Table \ref{table:decaydata}) such that significant proportions of these isotopes will decay during this time delay, hinting that these phenomena could be related. \\
...
Calculations were performed using the CASTEP 8.0 density functional theory (DFT) based code \cite{Clark2005}. Ultra-soft pseudo-potentials with a cut-off energy of 600 eV were employed. The exchange correlation functional was described by the Perdew, Burke and Ernzerhof (PBE) \cite{Perdew1996} parameterisation of the generalised gradient approximation (GGA). A Monkhorst-Pack sampling scheme \cite{Monkhorst1976} was used for Brillouin zone integration, with a minimum \emph{k}-point separation of 0.09 \r{A}\textsuperscript{-1}. The Pulay method for density mixing \cite{Pulay1980} was used to improve simulation convergence. \\
...
Tellurium on an oxygen site adopts a 0 charge state, which while not obvious (it may be expected to act like oxygen, also a group 6 element), can be understood when considering chemical behaviour. Oxygen on its native site will readily adopt a charge state of -2. Tellurium on the other hand, exhibits more metallic behaviour, preferring to be oxidised itself (the electronegativities of oxygen and tellurium are 3.5 and 2.1 respectively). However, the \ch{V_{O}^{**}} site is highly reducing, and will counter the tellurium ion's proclivity to oxidise. The overall effect on tellurium in this case is to maintain its original electronic structure. Finally, when the tellurium concentration is increased to $10^{-3}$ parts/fu, greater occupancy on the zirconium site is predicted above oxygen pressures of $10^{-17}$ atm, forming the defect \ch{Te_{Zr}^{'''}}. This corresponds to a tellurium charge state of +1. While we would expect an even more positive charge state for tellurium, in this case it is the dominant negative defect by an order of magnitude and is necessary to charge compensate positive hole and \ch{Te_{O}^{**}} defects. \\
...
As fission products proceed down the decay chain from Te to Xe, the defects they produce in \zirconia\ have progressively larger volumes. The change in defect volume when \ch{Te_{Zr}^{'''}} decays and produces \ch{I_{Zr}^{'''}} is +27.14 \r{A}$^{3}$. This then decays and produces the defect \ch{Xe_{Zr}^{''''}}, which leads to an even larger change in defect volume of +28.43 \r{A}$^{3}$. Similarly, at the oxygen site, \ch{Te_{O}^{**}} decays to produce \ch{I_{O}^{*}}, with a resulting defect volume change of +27.16 \r{A}$^{3}$. The lattice mismatch of these defects in \zirconia\ will generate stresses and if produced in significant quantities could promote crack formation, exposing Zr metal. If enough fission products are implanted simultaneously, the rate of crack formation may become faster than new passivating oxide can be produced, and typical I-SCC mechanisms will take over to failure. This process may be a contributing factor to fuel cladding failures observed after a power ramp, with reported time delays between ramp and failure aligning with the decay rate of Te and I isotopes. 
...
\begin{figure}
    \centering
    \includegraphics[height=8cm]{img/te_decay_chain2.png}
    \caption{Diagram of the Te$_{135}$ decay chain with decay modes and half-lives shown.}
...
\begin{tikzpicture}
	\begin{axis}
		[width=8.22cm, xlabel={\ch{log_{10}}($p_{O_{2}}$) (atm)}, ylabel={\ch{log_{10}}([D]) (per f.u.)}, ymin=-10, ymax=0, xmin=-35, xmax=0, legend style={{draw=}, at={(0.37,0.965)}, anchor=north west, legend columns=3, nodes={scale=0.75, transform shape}}]
...
\node at (-33.7,-0.5) {\textbf{b)}};
			\end{axis}            
\end{tikzpicture}
		\caption{Brouwer diagrams for tetragonal phase \zirconia\ point defects at iodine concentrations of a) $10^{-5}$ and b) $10^{-3}$, at a temperature of 1500 K.}
...
\begin{tikzpicture}
	\begin{axis}
		[width=8.22cm, xlabel={\ch{log_{10}}($p_{O_{2}}$) (atm)}, ylabel={\ch{log_{10}}([D]) (per f.u.)}, ymin=-10, ymax=0, xmin=-35, xmax=0, legend style={{draw=}, at={(0.35,0.97)}, anchor=north west, legend columns=3, nodes={scale=0.75, transform shape}}]
...
\begin{tikzpicture}
	\begin{axis}
		[width=8.22cm, xlabel={\ch{log_{10}}($p_{O_{2}}$) (atm)}, ylabel={\ch{log_{10}}([D]) (per f.u.)}, ymin=-10, ymax=0, xmin=-35, xmax=0, legend style={{draw=}, at={(0.37,0.97)}, anchor=north west, legend columns=3, nodes={scale=0.75, transform shape}}]
...
\node at (-33.7,-0.5) {\textbf{b)}};
			\end{axis}            
\end{tikzpicture}
		\caption{Brouwer diagrams for tetragonal phase \zirconia\ point defects at caesium concentrations of a) $10^{-5}$ and b) $10^{-3}$, at a temperature of 1500 K.} %Space charge = 0}
...
\begin{table}[htp]
\centering
\caption{Defect volumes of the dominant defect types in tetragonal \zirconia .}
\label{defect_volumes}
\begin{tabular}{ccc}
\hline
Defect type & \begin{tabular}[c]{@{}c@{}}Defect volume relative\\ to perfect crystal (\r{A}$^{3}$)\end{tabular}  \\ \hline
\ch{V_{O}^{**}} & -40.45  \\ %7440 \\ %362 \\
\ch{V_{Zr}^{''''}} & 88.93 \\ %9072 \\ %8347 \\
%\ch{V_{O}^{*}} & -21.14 \\ %3522 \\
%\ch{V_{O}^{x}} & -2.96 \\ %415 \\
%\ch{Te_{O}^{***}} & -18.83 \\ %8097 \\ %6605 \\
\ch{Te_{O}^{**}} & 2.74 \\ %5940 \\ %3752 \\
\ch{Te_{O}^{*}} & 24.61 \\ %5090 \\ %7451 \\
%\ch{Te_{Zr}^{''''}} & 83.01 \\ %6289 \\ %8247 \\
\ch{Te_{Zr}^{'''}} & 59.93 \\ %0934 \\ %2232 \\
%\ch{I_{O}^{***}} & -18.59  \\ %6196 \\ %0449 \\ % fixed
\ch{I_{O}^{*}} & 29.90 \\ %8003 \\ %6407 \\ % fixed 
\ch{I_{Zr}^{'''}} & 87.07 \\ %5444 \\ %912 \\ % fixed
\ch{Xe_{O}^{**}} & 9.02  \\ %8234 \\ %3892 \\
%\ch{Xe_{O}^{*}} & 32.07 \\ %7508 \\ %5247 \\
\ch{Xe_{Zr}^{''''}} & 115.50 \\ %2321 \\ %4415 \\
\ch{Xe_{Zr}^{'''}} & 91.65 \\ %1595 \\ %578 \\
\ch{Cs_{O}^{**}} & 4.38 \\ %0571 \\ %1495 \\
\ch{Cs_{Zr}^{'''}} & 90.37 \\ \hline %0931 \\ \hline %6229 \\ \hline
\end{tabular}
\end{table}
...
%\begin{tabular}{ccc}
%\hline
%Defect type & \begin{tabular}[c]{@{}c@{}}Defect volume relative\\ to perfect %crystal (\r{A}$^{3}$)\end{tabular} & \begin{tabular}[c]{@{}c@{}}Formation %energy \\ at 1500 K (eV)\end{tabular} \\ \hline
%\ch{V_{O}^{**}} & -40.45 & -0.92 \\ %7440 \\ %362 \\
%\ch{V_{Zr}^{''''}} & 88.93 & 7.04 \\ %9072 \\ %8347 \\
%%\ch{V_{O}^{*}} & -21.14 \\ %3522 \\
%%\ch{V_{O}^{x}} & -2.96 \\ %415 \\
%\ch{Te_{O}^{***}} & -18.83 & 1.07 \\ %8097 \\ %6605 \\
%\ch{Te_{O}^{**}} & 2.74 & 2.24 \\ %5940 \\ %3752 \\
%\ch{Te_{O}^{*}} & 24.61 & 5.10 \\ %5090 \\ %7451 \\
%\ch{Te_{Zr}^{''''}} & 83.01 & 8.85 \\ %6289 \\ %8247 \\
%\ch{Te_{Zr}^{'''}} & 59.93 & 5.97 \\ %0934 \\ %2232 \\
%\ch{I_{O}^{***}} & -18.59 & -1.90 \\ %6196 \\ %0449 \\ % fixed
%\ch{I_{O}^{*}} & 29.90 & 0.89 \\ %8003 \\ %6407 \\ % fixed 
%\ch{I_{Zr}^{'''}} & 87.07 & 4.96 \\ %5444 \\ %912 \\ % fixed
%\ch{Xe_{O}^{**}} & 9.02 & -1.18 \\ %8234 \\ %3892 \\
%\ch{Xe_{O}^{*}} & 32.07 & 1.84 \\ %7508 \\ %5247 \\
%\ch{Xe_{Zr}^{''''}} & 115.50 & 6.72 \\ %2321 \\ %4415 \\
%\ch{Xe_{Zr}^{'''}} & 91.65 & 4.97 \\ %1595 \\ %578 \\
%\ch{Cs_{O}^{**}} & 4.38 & 4.17 \\ %0571 \\ %1495 \\
%\ch{Cs_{Zr}^{'''}} & 90.37 & 5.97 \\ \hline %0931 \\ \hline %6229 \\ \hline
%\end{tabular}
%\end{table}

\begin{table}
\caption{Independent fission product yields and half-lives for the major iodine isotopes and precursors in a thermal neutron reactor. Yields from the Joint Evaluated Fission and Fusion File (JEFF 3.3). All isotopes undergo single $\beta$- decay. Metastable states are included.}  \label{table:decaydata}
\begin{center}
\begin{tabular}{c c c}
\hline
...
\texorpdfstring{Te\textsubscript{135}}{Te135} & 3.4777 & 19.0 s \cite{Singh2008} \\
\texorpdfstring{I\textsubscript{135}}{I135} & 2.4635 & 6.58 h \cite{Singh2008} \\ 
\texorpdfstring{Te\textsubscript{136}}{Te136} & 1.6652 & 17.63  s \cite{Mccutchan2018NuclearA=136} \\
\texorpdfstring{I\textsubscript{136}}{I136} & 3.03823 & 83.4 s \cite{Mccutchan2018NuclearA=136} \\
